#!/usr/bin/env bash
# Live registration + connect all DBs — headed browser, real stack, keeps recording.
# No login/logout — first-time integration flow only.
set -euo pipefail
...
# Loads LIVE_DB_* from rails-enterprise/.env then .env.local.
# BYOD_* URLs in .env.local win over USERS/ACCOUNTS/LEDGER/AUDIT_DATABASE_URL (separate Neon branch for RAI-49).
...
if [[ -z "${LIVE_DB_ACCOUNTS_URL:-}" || -z "${LIVE_DB_USERS_URL:-}" || -z "${LIVE_DB_LEDGER_URL:-}" || -z "${LIVE_DB_AUDIT_URL:-}" ]]; then
  echo "Missing LIVE_DB_* URLs. Set BYOD_* or USERS/ACCOUNTS/LEDGER/AUDIT_DATABASE_URL in $ENTERPRISE/.env or .env.local" >&2
  exit 1
fi
...
echo "=== Live BYOD registration + connect (headed) ==="
echo "Gateway:  $GATEWAY_URL"
echo "BFF:      $BFF_URL"
echo "Site:     $SITE_URL"
echo "Bug log:  $WEB/e2e/artifacts/byod-journey-live-bugs.jsonl"
echo "Timeline: $WEB/e2e/artifacts/byod-journey-live-timeline.jsonl"
echo ""
...
if ! curl -sf "$SITE_URL" >/dev/null 2>&1; then
  echo "Starting rails-web on $SITE_URL (NEXT_PUBLIC_CLIENT_SERVER=$BFF_URL) ..."
  (
    cd "$WEB"
    export PORT=3000
  ...
  echo "WARNING: if connects fail, restart web with NEXT_PUBLIC_CLIENT_SERVER=$BFF_URL" >&2
fi
...
echo ""
echo "Opening headed browser — register + connect all DBs (no login) ..."
npx playwright test e2e/byod-registration-connect.live.spec.ts \
  --config=playwright.live.config.ts \
  --workers=1 \
  --headed

echo ""
echo "Recording: find $WEB/recordings-output -name 'video.webm'"
echo "Bug log:   $WEB/e2e/artifacts/byod-journey-live-bugs.jsonl"
echo "Timeline:  $WEB/e2e/artifacts/byod-journey-live-timeline.jsonl"
